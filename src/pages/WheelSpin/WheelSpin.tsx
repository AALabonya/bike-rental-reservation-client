...
//   const handleSpin = () => {
//     if (isSpinning || !couponsData?.data?.length) return; // Check if data exists and has length
//     setIsSpinning(true);
//     const newRotation = rotation + 360 * 5 + Math.floor(Math.random() * 360);
//     setRotation(newRotation);
...
export default function WheelSpin() {
  ...
  if (isError) return <p>Error fetching coupons</p>;
  ...
  const handleSpin = () => {
    ...
    const length = couponsData.data.length;
    const selectedIndex = Math.floor((newRotation % 360) / (360 / length));

    setTimeout(() => {
      if (couponsData.data) {
        const coupon = couponsData.data[length - 1 - selectedIndex];
        setSelectedCoupon(coupon || null); // Fallback to null if coupon is undefined
        setIsSpinning(false);
        setShowCoupon(true);
      }
    }, 5000);
    ...
    <div className="overflow-hidden">
      ...
      <div className="flex flex-col items-center justify-center py-16 bg-white dark:bg-gray-900 p-4 overflow-hidden">
        <div className="flex flex-col lg:flex-row justify-center lg:justify-around border p-2 gap-11 lg:gap-28 items-center w-full overflow-hidden">
          ...
          <div className="wheel-container overflow-hidden">
            <div
              className="wheel overflow-hidden"
              style={{ transform: `rotate(${rotation}deg)` }}
            >
              {couponsData?.data?.map((coupon: TCoupon, index: number) => (
                <div
